...
const HierarchyList = ({
  ...
  console.log('selected items: ' + JSON.stringify(editModeSelectedIds));

  const handleSelect = (id, parentId = null) => {
    if (editingStyle) {
      console.log('newly selected: ' + id);
  ...
  const handleMove = (dragId, hoverId, target) => {
    let updatedList;
  ...
  return (
    <>
      <HierarchyListReorderModal
        ...
        onSubmit={target => {
          if (target !== null) {
            handleMove(selectedIds[0], target, DropLocation.Nested);
          }
